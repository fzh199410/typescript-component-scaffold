...
gulp.task('build-css', () => {
    ...
    var lessStream = gulp.src(`${SRC_PATH}/**/*.@(less)`)
                         .pipe(less());
    return eventStream.merge(lessStream, gulp.src(`${SRC_PATH}/**/*.@(css)`))
                //    .pipe(cssBase64())       // css引用的图片转成base64格式
                      .pipe(gulp.dest(DEST_PATH));
});
// 编译成 es module 或 commonjs 格式(根据BABEL_ENV参数), 引用的图片转换为base64格式
gulp.task('build-js', () => {
    return gulp.src([`${SRC_PATH}/**/*.@(js|ts|tsx)`, `!${SRC_PATH}/dev.js`])
               .pipe(babel())
               .pipe(replace(/\.ts(x?)$/g, '.js'))                   // 替换 jsx 文件名和文件内的引用名
               .pipe(replace(/\.(less|scss)/g, '.css'))     // 替换 less, scss 文件名和 js 文件中引用的 scss 文件名
               .pipe(gulp.dest(DEST_PATH));
});
// 复制图片
...
gulp.task('watch', (done) => {
    gulp.watch([`${SRC_PATH}/**/*.@(js|tsx)`], { delay: WATCH_DELAY },  gulp.series('build-js'));
    gulp.watch([`${SRC_PATH}/**/*.@(css|scss)`], { delay: WATCH_DELAY },  gulp.series('build-css'));
